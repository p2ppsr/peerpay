...
A React UI for **peer-to-peer Bitcoin payments** built on top of [PeerPay](https://github.com/bitcoin-sv) and the BSV blockchain. **PeerPay-React** provides a simple, material-designed interface for sending payments, viewing incoming payments, and interacting with the underlying PeerPayClient. It leverages components from `@mui/material` and `@bsv/identity-react` to offer a seamless payment experience.
